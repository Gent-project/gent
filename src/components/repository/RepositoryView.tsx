...
import GitOperationsModal from "@/app/dashboard/repository/[owner_id]/[repo_name]/_components/GitOperationsModal";
import CommitsTab from "@/app/dashboard/repository/[owner_id]/[repo_name]/_components/CommitsTab";
import BranchesTab from "@/app/dashboard/repository/[owner_id]/[repo_name]/_components/BranchesTab";
...
export interface RepositoryViewProps {
  /** Numeric user id or username — the API resolves either. */
  owner: number | string;
  repoName: string;
    ...
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
        <div className="rounded-2xl border p-10 text-center" style={{ background: t.elevated, borderColor: t.border }}>
          ...
          <Link href={notFoundHref ?? backHref} className="mt-5 inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold" style={{ background: t.accent, color: t.successText }}>
  ...
  return (
    <div className="mx-auto max-w-[1380px] px-4 py-5 sm:px-6 sm:py-7">
      <nav className="mb-4 flex min-w-0 items-center gap-1.5 text-xs" style={{ color: t.textMuted }} aria-label="Repository breadcrumb">
        <Link href={backHref} className="inline-flex items-center gap-1.5 hover:underline"><ArrowLeft className="h-3.5 w-3.5" /> {backLabel}</Link>
      ...
      <motion.header initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-2xl border" style={{ background: t.elevated, borderColor: t.border }}>
        <div className="flex flex-col gap-5 p-5 sm:p-6 lg:flex-row lg:items-start lg:justify-between">
          ...
          {repository.role === "owner" && (
            <Link href={`/dashboard/repository/${repository.owner_id}/${repoName}/settings`} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg border px-3.5 py-2 text-sm font-medium" style={{ borderColor: t.border, color: t.text, background: t.inputBg }}>
      ...
      <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-5 grid items-start gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-2xl border p-4" style={{ background: t.elevated, borderColor: t.border }}>
            ...
            {isPublic && (
              <p className="mt-2 text-[11px] leading-4" style={{ color: t.textMuted }}>
                This clone URL is the legacy REST endpoint, which requires
                <span className="font-mono"> gent login</span>. Sign in to clone it.
              </p>
            )}
      ...
      {!isPublic && <GitOperationsModal isOpen={showGitOpsModal} onClose={() => setShowGitOpsModal(false)} ownerId={ownerId} repoName={repoName} isDark={isDark} repositoryUrl={cloneUrl} defaultBranch={repository.default_branch} />}
    </div>
  );
}
