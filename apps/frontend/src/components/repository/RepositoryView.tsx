...
export default function RepositoryView({
  ...
  const readableOwner = repository ? ownerId : "";
  const { data: branches = [], isLoading: branchesLoading } = useBranches(readableOwner, repoName);
  const { data: commits = [], isLoading: commitsLoading } = useCommits(readableOwner, repoName);
  const { data: tags = [], isLoading: tagsLoading } = useTags(readableOwner, repoName);
  ...
  return (
    <div className="mx-auto max-w-[1380px] px-4 py-5 sm:px-6 sm:py-7">
      ...
      <GitOperationsModal canWrite={canWrite} isOpen={showGitOpsModal} onClose={() => setShowGitOpsModal(false)} ownerId={ownerId} repoName={repoName} isDark={isDark} repositoryUrl={cloneUrl} defaultBranch={repository.default_branch} objectFormat={repository.object_format} />
    </div>
  );
}
