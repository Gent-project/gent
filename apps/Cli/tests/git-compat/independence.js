...
const modules = ['migrate', 'smart-http', 'attributes', 'feature-support', 'gent-ops', 'git-config', 'git-index', 'git-objects', 'ignore', 'lockfile', 'merge-ops', 'object-store', 'packfile', 'refs', 'repository', 'stash-ops', 'worktree', 'canonical-journal'];
...
const result = spawnSync(process.execPath, ['--test', path.join(__dirname, 'engine.test.js'), path.join(__dirname, 'cli.test.js'), path.join(__dirname, 'transport.test.js'), path.join(__dirname, 'migrate.test.js')], { env, stdio: 'inherit' });
